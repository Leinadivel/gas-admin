  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard label="Pending Orders" value={loading ? '…' : String(pendingOrders)} hint="status = pending" />
        <StatCard label="Active Vendors" value={loading ? '…' : String(activeVendors)} hint="is_online = true" />
        <StatCard label="Transactions" value={loading ? '…' : String(totalTransactions)} />
      </div>

      <div className="grid gap-4 lg:grid-cols-2">
        <ChartCard title="Orders by status" subtitle="Distribution of order statuses" series={statusSeries} />
        <ChartCard title="Transactions by type" subtitle="Distribution of transaction types" series={txSeries} />
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-xl border bg-white p-4">
          <div className="flex items-center justify-between">
            ...
            <Link href="/orders" className="text-xs underline underline-offset-2">View all</Link>
          </div>
        ...
        <div className="rounded-xl border bg-white p-4">
          <div className="flex items-center justify-between">
            ...
            <Link href="/transactions" className="text-xs underline underline-offset-2">View all</Link>
          </div>
              ...
              recentTx.map((t) => (
                <div key={t.id} className="py-3">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="text-xs">{t.type} • {String(t.amount)}</div>
                  </div>

                  <div className="mt-1 flex items-center justify-between text-xs opacity-60">
                    <div>{t.status ?? '—'}</div>
                    {t.order_id ? (
                      <Link href={`/orders/${t.order_id}`} className="underline underline-offset-2">order</Link>
                    ) : (
                      <span>—</span>
                    )}
                  </div>
...
function ChartCard({ title, subtitle, series }: { title: string; subtitle: string; series: { label: string; value: number }[] }) {
          ...
          series.map((s) => (
            <div key={s.label} className="space-y-1">
              ...
              <div className="h-2 w-full rounded-full bg-gray-100 overflow-hidden">
                <div className="h-2 rounded-full bg-gray-900" style={{ width: `${Math.round((s.value / max) * 100)}%` }} />
              </div>
            </div>
          ))
        )}
